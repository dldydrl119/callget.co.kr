      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            <h3><?=$name?></h3>
            <?if($insert) {?>
                <a href = "<?=$insert?>"><button type="button" class="btn btn-info" data-url="/ad03/admin/yutubeCount">등록하기</button></a>
            <?}?>
		          <button type="button" id="idxgo" class="btn btn-primary" data-url="/ad03/admin/yutubeIdxsUpdate" data-count="/ad03/admin/yutubeCount" data-page="<?=$page?>">저장하기</button></a>
            <div class="clearfix"></div>
          </div>
          <form>
          <div class="x_content">
...
<script>
    $('body').on('click', '.adminedit', function() {
        ...
        location.href = '/ad03/admin/yutubeedit/'+idx;
    });
    $('body').on('click', '.admindel', function() {
        var con = confirm("정말 삭제하시겠습니까?");
        if(con) {
            var idx = $(this).parent().parent().attr('data-idx');
            var result = jsonreturn('/ad03/admin/yutubedel/'+idx);
