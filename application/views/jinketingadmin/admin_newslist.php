    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        <div class="x_title">
          ...
          <?}?>
          <button type="button" id="idxgo" class="btn btn-primary" data-url="/jinketing/admin/newsIdxsUpdate" data-count="/jinketing/admin/newsCount" data-page="<?= $page ?>">저장하기</button></a>
          <div class="clearfix"></div>
        </div>
        <div class="x_content">
            ...
            <table id="datatable" class="table table-striped table-bordered">
              ...
              <tbody>
                ...
                <tr class="even pointer" data-idx="<?= $list[$i]->news_idx ?>">
                  ...
                  <td> <button type="button" class="btn btn-info btn-show" data-value="<?= $value ?>" data-url="/jinketing/admin/newsShow">
                      <?if($value == 1){ $text = "숨기기";} else { $text = "보이기";} echo $text;?>
                    </button> </td>
                  <?}
            				$j++;}?>
                  <?= $adminbutton ?>
...
<script>
  $('body').on('click', '.adminedit', function() {
    ...
    location.href = '/jinketing/admin/newsedit/' + idx;
  });
  $('body').on('click', '.admindel', function() {
    var con = confirm("정말 삭제하시겠습니까?");
    if (con) {
      var idx = $(this).parent().parent().attr('data-idx');
      var result = jsonreturn('/jinketing/admin/newsdel/' + idx);
