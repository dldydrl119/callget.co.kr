        ...
        <div class="x_panel">
          <div class="x_title">
            <h3><?=$name?></h3>
              ...
              <?}?>
		          <button type="button" id="idxgo" class="btn btn-primary" data-url="/ad02/admin/newsIdxsUpdate" data-count="/ad02/admin/newsCount" data-page="<?=$page?>">저장하기</button></a>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
                    ...
                    <tr class="even pointer" data-idx="<?=$list[$i]->news_idx?>">
                      ...
                      if($key != "shows") {
                        ...
                        if(!strpos($key,'idx')) {
                          ...
                          <td> <button type="button" class="btn btn-info btn-show" data-value="<?=$value?>" data-url="/ad02/admin/newsShow"> <?if($value == 1){ $text = "숨기기";} else { $text = "보이기";} echo $text;?> </button> </td>
                    <?}
            				$j++;}?>
                        <?=$adminbutton?>
                    </tr>
...
<script>
    $('body').on('click', '.adminedit', function() {
        ...
        location.href = '/ad02/admin/newsedit/'+idx;
    });
    $('body').on('click', '.admindel', function() {
        var con = confirm("정말 삭제하시겠습니까?");
        if(con) {
            var idx = $(this).parent().parent().attr('data-idx');
            var result = jsonreturn('/ad02/admin/newsdel/'+idx);
