      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            ...
            <?}?>
		          <button type="button" id="idxgo" class="btn btn-primary" data-url="/wemakeprice/admin/faqIdxsUpdate" data-count="/wemakeprice/admin/faqCount" data-page="<?=$page?>">저장하기</button></a>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
            <div class="table-responsive">
              <table id="datatable" class="table table-striped table-bordered">
                ...
                <?for($i = 0; $i < count($list); $i++) {$j=0;?>
                    <tr class="even pointer" data-idx="<?=$list[$i]->faq_idx?>">
...
<script>
    $('body').on('click', '.adminedit', function() {
        ...
        location.href = '/wemakeprice/admin/faqedit/'+idx;
    });
    $('body').on('click', '.admindel', function() {
        var con = confirm("정말 삭제하시겠습니까?");
        if(con) {
            var idx = $(this).parent().parent().attr('data-idx');
            var result = jsonreturn('/wemakeprice/admin/faqdel/'+idx);
