<?php
class Wemakeprice_adminmodel extends CI_Model {
    ...
    public function insert_login() {
        ...
        $result = $this->db->insert('wemakeprice_admin', $array);
    ...
    public function loginData() {
        // $_POST['id'] = 'wemakeprice';
        // $_POST['pass'] = 'admin';
        $sql="SELECT admin_idx, admin_id, admin_pass FROM wemakeprice_admin WHERE admin_id = ?";
        $array = array($_POST['id']);
        $data = $this->db->query($sql, $array)->row();

        if (password_verify($_POST['pass'] ,$data->admin_pass) ) {

            setcookie('wemakeprice_id', $_POST['id'], time() + 3600);
            setcookie('wemakeprice_idx', $data->admin_idx, time() + 3600);
...
//slide -----------------------------------------------------------------
    ...
    public function slideinsertData() {
        ...
        $sql="SELECT slide_idxs FROM wemakeprice_slide order by slide_idxs DESC LIMIT 1";
    ...
    public function slideIdxsUpdate() {
        ...
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE wemakeprice_slide SET slide_idxs = ? WHERE slide_idx = ?";
    ...
    public function slidedel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM wemakeprice_slide WHERE slide_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }

    public function slideCount() {
        $sql="SELECT COUNT(*) count  FROM wemakeprice_slide";
...
//-------------------------------------------------------------------------
    ...
    public function categoryData($idx) {
        ...
        $sql="SELECT category_idx, category_name, category_image
            FROM wemakeprice_category WHERE category_idx = ?";
        return $this->db->query($sql, $array)->row();
    }

    public function categoryUpdate() {
        $array = array($_POST['name'], $_POST['image'], $_POST['idx']);
        $sql= "UPDATE wemakeprice_category SET category_name = ?, category_image = ?
                WHERE category_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function categorydel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM wemakeprice_category WHERE category_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function faqinsertData() {
        $table = "wemakeprice_faq";
        $sql="SELECT faq_idxs FROM wemakeprice_faq order by faq_idxs DESC LIMIT 1";
    ...
    public function faqListData($page = 1) {
        ...
        $sql="SELECT faq_idx, @rownum:=@rownum+1 num, faq_question, faq_answer
                FROM wemakeprice_faq, (SELECT @rownum:=0) TMP
    ...
    public function faqData($idx) {
        ...
        $sql="SELECT faq_idx, faq_question, faq_answer
            FROM wemakeprice_faq WHERE faq_idx = ?";
        return $this->db->query($sql, $array)->row();
    }

    public function faqUpdate() {
        $array = array($_POST['question'], $_POST['answer'], $_POST['idx']);
        $sql= "UPDATE wemakeprice_faq SET faq_question = ?, faq_answer = ?
                WHERE faq_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function faqIdxsUpdate() {
        ...
        $table = "wemakeprice_faq";
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE wemakeprice_faq SET faq_idxs = ? WHERE faq_idx = ?";
            $array = array($idxs[$i], $idx[$i]);
            $result = $this->db->query($sql, $array);
        }
        return array('return'=>$result);
    ...
    public function faqdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM wemakeprice_faq WHERE faq_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }

    public function faqCount() {
        $sql="SELECT COUNT(*) count  FROM wemakeprice_faq";
        return $this->db->query($sql, $array)->row()->count;
    }

    //상품 관리
    public function productinsertData() {
        $producttable = "wemakeprice_product";
        $insuretable = "wemakeprice_insure";

        $sql="SELECT product_idxs FROM wemakeprice_product order by product_idxs DESC LIMIT 1";
    ...
    public function categorySelectData() {
        $sql="SELECT category_idx, category_name FROM wemakeprice_category";
    ...
    public function productListData($page = 1) {
        ...
        SELECT product_idx
            , category_name, insure_name, insure_call,
                ...
                product_show shows
                 FROM wemakeprice_product
        				JOIN wemakeprice_insure USING(product_idx)
        				LEFT JOIN wemakeprice_category
        				USING(category_idx)  JOIN ( SELECT @rownum := 0) R
          ) A ORDER BY product_idxs DESC
              limit {$limit} offset {$offset}";
        $result = $this->db->query($sql)->result();

        $sql="SELECT COUNT(*) count FROM wemakeprice_product
        JOIN wemakeprice_insure USING(product_idx) JOIN wemakeprice_category
        USING(category_idx)";
        $count = $this->db->query($sql)->row();
    ...
    public function productData($idx) {
        $sql="SELECT product_idx, category_idx, insure_name, insure_call,
              ...
              FROM wemakeprice_product JOIN wemakeprice_insure
    ...
    public function productIdxUpdate() {
        $idxs = $_POST['idxs'];
        $idx = $_POST['idx'];
        $table = "wemakeprice_product";
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE wemakeprice_product SET product_idxs = ? WHERE product_idx = ?";
            $array = array($idxs[$i], $idx[$i]);
            $result = $this->db->query($sql, $array);
        }
        return array('return'=>$result);
    ...
    public function productdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM wemakeprice_product WHERE product_idx = ?";
        $result = $this->db->query($sql, $array);
        $sql= "DELETE FROM wemakeprice_insure WHERE product_idx = ?";
    ...
    public function productShow() {
        ...
        $sql= "UPDATE wemakeprice_product SET product_show = ?
          WHERE product_idx = ?";
...
//소식---------------------------------------------------------
    public function newsListData($page = 1) {
        ...
        $sql="SELECT news_idx, @rownum:=@rownum+1 num, news_title, news_thumb image,
                ...
                FROM wemakeprice_news, (SELECT @rownum:=0) TMP
                ORDER BY news_idxs DESC
                limit {$limit} offset {$offset}";
        // limit {$limit} offset {$offset}
        $result = $this->db->query($sql)->result();

        $sql="SELECT COUNT(*) count FROM wemakeprice_news";
        $count = $this->db->query($sql)->row();
    ...
    public function newsInsertData() {
        $sql = "INSERT INTO wemakeprice_news (news_title, news_content, news_thumb, news_date) VALUES (?, ?, ?, now())";
        $array = array($_POST['news_title'],$_POST['news_content'],$_POST['news_thumb']);
        $result = $this->db->query($sql,$array);
        $insert_id = $this->db->insert_id();

        $sql="SELECT news_idxs FROM wemakeprice_news order by news_idxs DESC LIMIT 1";
        $news_idxs = $this->db->query($sql)->row()->news_idxs + 1;

        $sql= "UPDATE wemakeprice_news SET news_idxs = ? WHERE news_idx = ?";
    ...
    public function newsShow() {
        ...
        $sql= "UPDATE wemakeprice_news SET news_show = ?
        WHERE news_idx = ?";
        $array = array($show, $_POST['idx']);
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    ...
    public function newsData($idx) {
        $sql="SELECT news_idx, news_title, news_content, news_thumb
            FROM wemakeprice_news WHERE news_idx = ?";
        $array = array($idx);
        return $this->db->query($sql, $array)->row();
    }
    ...
    public function newsUpdate() {
        ...
        $sql= "UPDATE wemakeprice_news SET news_title = ?, news_content = ?, news_thumb = ?
              WHERE news_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function newsDel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM wemakeprice_news WHERE news_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
	...
	public function hyunTest() {
        $sql="SELECT * FROM wemakeprice_product";
        $result = $this->db->query($sql)->result();;
		return $result;
    }

    public function yutubelistData($page = 1) {
        $table = "wemakeprice_yutube";
        $limit=10;
        $offset=$limit*($page-1);
        $sql="SELECT yutube_idx, @rownum:=@rownum+1 num, yutube_link, yutube_date
                FROM wemakeprice_yutube, (SELECT @rownum:=0) TMP
    ...
    public function yutubeinsertData() {
        ...
        $sql="SELECT yutube_idxs FROM wemakeprice_yutube order by yutube_idxs DESC LIMIT 1";
    ...
    public function yutubeData($idx) {
        ...
        $sql="SELECT yutube_idx, yutube_name, yutube_image, yutube_link
            FROM wemakeprice_yutube WHERE yutube_idx = ?";
        return $this->db->query($sql, $array)->row();
    }

    public function yutubeUpdate() {
        $array = array($_POST['name'], $_POST['image'], $_POST['link'], $_POST['idx']);
        $sql= "UPDATE wemakeprice_yutube SET yutube_name = ?, yutube_image = ?,
    ...
    public function yutubeIdxsUpdate() {
        ...
        $table = "wemakeprice_yutube";
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE wemakeprice_yutube SET yutube_idxs = ? WHERE yutube_idx = ?";
            $array = array($idxs[$i], $idx[$i]);
            $result = $this->db->query($sql, $array);
        }
        return array('return'=>$result);
    ...
    public function yutubedel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM wemakeprice_yutube WHERE yutube_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }

    public function yutubeCount() {
        $sql="SELECT COUNT(*) count  FROM wemakeprice_yutube";
        return $this->db->query($sql, $array)->row()->count;
    }
}
