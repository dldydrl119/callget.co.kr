<?php
class Ad05model extends CI_Model {
    ...
    public function yutubeData() {   //메인 슬라이드
        $sql="SELECT yutube_idx, yutube_link
         FROM ad05_yutube ORDER BY yutube_idxs DESC";
    ...
    public function SlideData() {   //메인 슬라이드
        $sql="SELECT slide_idx, slide_image, slide_link, slide_name
         FROM ad05_slide ORDER BY slide_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
...
//                product_hashtag FROM ad05_product
...
//                limit {$limit} offset {$offset} ";
    ...
    public function  callData($idx) {
        $sql="SELECT product_idx, product_slide, product_name, insure_call,
                ...
                product_thumb,
                product_get FROM ad05_product JOIN ad05_insure
                USING(product_idx) WHERE product_idx = ?";
        $array= array($idx);
        $data = $this->db->query($sql, $array)->row();
        return array('return'=>true,'data'=>$data);
    ...
    public function  faqData() {
        $sql="SELECT faq_idx, faq_question, faq_answer FROM ad05_faq ORDER BY faq_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
    public function  updateGet($idx) {
        $array = array($idx);
        $sql="SELECT product_get FROM ad05_product WHERE product_idx = ?";
        $count = $this->db->query($sql, $array)->row()->product_get;
        $count += 1;
        $array = array($count, $idx);
        $sql="UPDATE ad05_product SET product_get = ? WHERE product_idx = ?";
...
////소식 페이지
    ...
    public function  newsDataView($idx) {
        ...
        $sql = "SELECT * FROM ad05_news WHERE news_idx = (SELECT MIN(news_idx) min FROM ad05_news WHERE news_idx > ? AND news_show = 1)";
        $array = array($idx);
        $min = $this->db->query($sql, $array)->row()->news_idx;

        $sql = "SELECT * FROM ad05_news WHERE news_idx = (SELECT MAX(news_idx) FROM ad05_news WHERE news_idx < ? AND news_show = 1)";
        $array = array($idx);
