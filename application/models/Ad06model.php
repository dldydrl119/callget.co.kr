<?php
class Ad06model extends CI_Model {
    ...
    public function yutubeData() {   //메인 슬라이드
        $sql="SELECT yutube_idx, yutube_link
         FROM ad06_yutube ORDER BY yutube_idxs DESC";
    ...
    public function SlideData() {   //메인 슬라이드
        $sql="SELECT slide_idx, slide_image, slide_link, slide_name
         FROM ad06_slide ORDER BY slide_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
...
//                product_hashtag FROM ad06_product
...
//                limit {$limit} offset {$offset} ";
    ...
    public function  getData($idx) {   //겟 데이터
        $sql="SELECT product_idx, CONCAT_WS(',',IF(product_slidevideo='',null,product_slidevideo),product_slide) product_slide,
                product_company, insure_company,insure_name, product_logo, insure_logo, product_name,
    ...
    public function  callData($idx) {
        $sql="SELECT product_idx, product_slide, product_name, insure_call,
                ...
                product_thumb,
                product_get FROM ad06_product JOIN ad06_insure
                USING(product_idx) WHERE product_idx = ?";
        $array= array($idx);
        $data = $this->db->query($sql, $array)->row();
        return array('return'=>true,'data'=>$data);
    ...
    public function  faqData() {
        $sql="SELECT faq_idx, faq_question, faq_answer FROM ad06_faq ORDER BY faq_idxs DESC";
        $list = $this->db->query($sql)->result();
        return array('return'=>true,'list'=>$list);
    }
    public function  updateGet($idx) {
        $array = array($idx);
        $sql="SELECT product_get FROM ad06_product WHERE product_idx = ?";
        $count = $this->db->query($sql, $array)->row()->product_get;
        $count += 1;
        $array = array($count, $idx);
        $sql="UPDATE ad06_product SET product_get = ? WHERE product_idx = ?";
...
////소식 페이지
    ...
    public function  newsDataView($idx) {
        ...
        $sql = "SELECT * FROM ad06_news WHERE news_idx = (SELECT MIN(news_idx) min FROM ad06_news WHERE news_idx > ? AND news_show = 1)";
        $array = array($idx);
        $min = $this->db->query($sql, $array)->row()->news_idx;

        $sql = "SELECT * FROM ad06_news WHERE news_idx = (SELECT MAX(news_idx) FROM ad06_news WHERE news_idx < ? AND news_show = 1)";
        $array = array($idx);
