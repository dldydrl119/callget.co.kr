<?php
class Jpgroup_adminmodel extends CI_Model {
    ...
    public function insert_login() {
        ...
        $result = $this->db->insert('jpgroup_admin', $array);
    ...
    public function loginData() {
        // $_POST['id'] = 'jpgroup';
        // $_POST['pass'] = 'admin';
        $sql="SELECT admin_idx, admin_id, admin_pass FROM jpgroup_admin WHERE admin_id = ?";
        $array = array($_POST['id']);
        $data = $this->db->query($sql, $array)->row();

        if (password_verify($_POST['pass'] ,$data->admin_pass) ) {

            setcookie('jpgroup_id', $_POST['id'], time() + 3600);
            setcookie('jpgroup_idx', $data->admin_idx, time() + 3600);
...
//slide -----------------------------------------------------------------
    public function slideListData($page = 1) {
        ...
        $sql="SELECT slide_idx, @rownum:=@rownum+1 num, slide_name, slide_image, slide_link, slide_date
                FROM jpgroup_slide, (SELECT @rownum:=0) TMP
    ...
    public function slideinsertData() {
        ...
        $sql="SELECT slide_idxs FROM jpgroup_slide order by slide_idxs DESC LIMIT 1";
    ...
    public function slideIdxsUpdate() {
        ...
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE jpgroup_slide SET slide_idxs = ? WHERE slide_idx = ?";
    ...
    public function slidedel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM jpgroup_slide WHERE slide_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }

    public function slideCount() {
        $sql="SELECT COUNT(*) count  FROM jpgroup_slide";
...
//-------------------------------------------------------------------------
    ...
    public function categoryData($idx) {
        ...
        $sql="SELECT category_idx, category_name, category_image
            FROM jpgroup_category WHERE category_idx = ?";
        return $this->db->query($sql, $array)->row();
    }

    public function categoryUpdate() {
        $array = array($_POST['name'], $_POST['image'], $_POST['idx']);
        $sql= "UPDATE jpgroup_category SET category_name = ?, category_image = ?
                WHERE category_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function categorydel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM jpgroup_category WHERE category_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function faqinsertData() {
        $table = "jpgroup_faq";
        $sql="SELECT faq_idxs FROM jpgroup_faq order by faq_idxs DESC LIMIT 1";
    ...
    public function faqListData($page = 1) {
        ...
        $sql="SELECT faq_idx, @rownum:=@rownum+1 num, faq_question, faq_answer
                FROM jpgroup_faq, (SELECT @rownum:=0) TMP
    ...
    public function faqData($idx) {
        ...
        $sql="SELECT faq_idx, faq_question, faq_answer
            FROM jpgroup_faq WHERE faq_idx = ?";
        return $this->db->query($sql, $array)->row();
    }

    public function faqUpdate() {
        $array = array($_POST['question'], $_POST['answer'], $_POST['idx']);
        $sql= "UPDATE jpgroup_faq SET faq_question = ?, faq_answer = ?
                WHERE faq_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function faqIdxsUpdate() {
        ...
        $table = "jpgroup_faq";
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE jpgroup_faq SET faq_idxs = ? WHERE faq_idx = ?";
            $array = array($idxs[$i], $idx[$i]);
            $result = $this->db->query($sql, $array);
        }
        return array('return'=>$result);
    ...
    public function faqdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM jpgroup_faq WHERE faq_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }

    public function faqCount() {
        $sql="SELECT COUNT(*) count  FROM jpgroup_faq";
        return $this->db->query($sql, $array)->row()->count;
    }

    //상품 관리
    public function productinsertData() {
        $producttable = "jpgroup_product";
        $insuretable = "jpgroup_insure";

        $sql="SELECT product_idxs FROM jpgroup_product order by product_idxs DESC LIMIT 1";
    ...
    public function categorySelectData() {
        $sql="SELECT category_idx, category_name FROM jpgroup_category";
    ...
    public function productListData($page = 1) {
        ...
        SELECT product_idx
            , category_name, insure_name, insure_call,
                ...
                product_show shows
                 FROM jpgroup_product
        				JOIN jpgroup_insure USING(product_idx)
        				LEFT JOIN jpgroup_category
        				USING(category_idx)  JOIN ( SELECT @rownum := 0) R
          ) A ORDER BY product_idxs DESC
              limit {$limit} offset {$offset}";
        $result = $this->db->query($sql)->result();

        $sql="SELECT COUNT(*) count FROM jpgroup_product
        JOIN jpgroup_insure USING(product_idx) JOIN jpgroup_category
        USING(category_idx)";
        $count = $this->db->query($sql)->row();
    ...
    public function productData($idx) {
        $sql="SELECT product_idx, category_idx, insure_name, insure_call,
              ...
              FROM jpgroup_product JOIN jpgroup_insure
    ...
    public function productIdxUpdate() {
        $idxs = $_POST['idxs'];
        $idx = $_POST['idx'];
        $table = "jpgroup_product";
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE jpgroup_product SET product_idxs = ? WHERE product_idx = ?";
            $array = array($idxs[$i], $idx[$i]);
            $result = $this->db->query($sql, $array);
        }
        return array('return'=>$result);
    ...
    public function productdel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM jpgroup_product WHERE product_idx = ?";
        $result = $this->db->query($sql, $array);
        $sql= "DELETE FROM jpgroup_insure WHERE product_idx = ?";
    ...
    public function productShow() {
        ...
        $sql= "UPDATE jpgroup_product SET product_show = ?
          WHERE product_idx = ?";
...
//소식---------------------------------------------------------
    public function newsListData($page = 1) {
        ...
        $sql="SELECT news_idx, @rownum:=@rownum+1 num, news_title, news_thumb image,
                ...
                FROM jpgroup_news, (SELECT @rownum:=0) TMP
                ORDER BY news_idxs DESC
                limit {$limit} offset {$offset}";
        // limit {$limit} offset {$offset}
        $result = $this->db->query($sql)->result();

        $sql="SELECT COUNT(*) count FROM jpgroup_news";
        $count = $this->db->query($sql)->row();
    ...
    public function newsInsertData() {
        $sql = "INSERT INTO jpgroup_news (news_title, news_content, news_thumb, news_date) VALUES (?, ?, ?, now())";
        $array = array($_POST['news_title'],$_POST['news_content'],$_POST['news_thumb']);
        $result = $this->db->query($sql,$array);
        $insert_id = $this->db->insert_id();

        $sql="SELECT news_idxs FROM jpgroup_news order by news_idxs DESC LIMIT 1";
        $news_idxs = $this->db->query($sql)->row()->news_idxs + 1;

        $sql= "UPDATE jpgroup_news SET news_idxs = ? WHERE news_idx = ?";
    ...
    public function newsShow() {
        ...
        $sql= "UPDATE jpgroup_news SET news_show = ?
        WHERE news_idx = ?";
        $array = array($show, $_POST['idx']);
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    ...
    public function newsData($idx) {
        $sql="SELECT news_idx, news_title, news_content, news_thumb
            FROM jpgroup_news WHERE news_idx = ?";
        $array = array($idx);
        return $this->db->query($sql, $array)->row();
    }
    ...
    public function newsUpdate() {
        ...
        $sql= "UPDATE jpgroup_news SET news_title = ?, news_content = ?, news_thumb = ?
              WHERE news_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
    ...
    public function newsDel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM jpgroup_news WHERE news_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }
	...
	public function hyunTest() {
        $sql="SELECT * FROM jpgroup_product";
        $result = $this->db->query($sql)->result();;
		return $result;
    }
    ...
    public function yutubeinsertData() {
        ...
        $sql="SELECT yutube_idxs FROM jpgroup_yutube order by yutube_idxs DESC LIMIT 1";
    ...
    public function yutubeData($idx) {
        ...
        $sql="SELECT yutube_idx, yutube_name, yutube_image, yutube_link
            FROM jpgroup_yutube WHERE yutube_idx = ?";
        return $this->db->query($sql, $array)->row();
    }

    public function yutubeUpdate() {
        $array = array($_POST['name'], $_POST['image'], $_POST['link'], $_POST['idx']);
        $sql= "UPDATE jpgroup_yutube SET yutube_name = ?, yutube_image = ?,
    ...
    public function yutubeIdxsUpdate() {
        ...
        $table = "jpgroup_yutube";
        for($i=0; $i<count($idxs); $i++) {
            $sql= "UPDATE jpgroup_yutube SET yutube_idxs = ? WHERE yutube_idx = ?";
            $array = array($idxs[$i], $idx[$i]);
            $result = $this->db->query($sql, $array);
        }
        return array('return'=>$result);
    ...
    public function yutubedel($idx) {
        $array = array($idx);
        $sql= "DELETE FROM jpgroup_yutube WHERE yutube_idx = ?";
        $result = $this->db->query($sql, $array);
        return array('return'=>$result);
    }

    public function yutubeCount() {
        $sql="SELECT COUNT(*) count  FROM jpgroup_yutube";
        return $this->db->query($sql, $array)->row()->count;
    }
}
