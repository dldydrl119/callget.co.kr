      ...
      <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
          <div class="x_title">
            ...
            <?}?>
		          <button type="button" id="idxgo" class="btn btn-primary" data-url="/ad02/admin/productIdxUpdate" data-count="/ad02/admin/productCount" data-page="<?=$page?>">저장하기</button></a>
            <div class="clearfix"></div>
          </div>
          <div class="x_content">
                        ...
                        <?foreach($pre_one[$i] as $key => $value) {
                            ...
                            if(!strpos($key,'idx')) {
                                if($key != "shows") {
                                    ...
                                    <td> <button type="button" class="btn btn-info btn-show" data-value="<?=$value?>" data-url="/ad02/admin/productShow"> <?if($value == 1){ $text = "숨기기";} else { $text = "보이기";} echo $text;?> </button> </td>
                                <?}
                            }
                            $j++;}?>
                        <?=$adminbutton?>
...
<? require_once(APPPATH."views/ad02admin/admin_footer.php");?>

<script src="/assets/js/ad02admin.js"></script>
<script src="/assets/js/common.js"></script>
<script>
    $('body').on('click', '.adminedit', function() {
        var idx = $(this).parent().parent().attr('data-idx');
        location.href = '/ad02/admin/productedit/'+idx;
    });
    $('body').on('click', '.admindel', function() {
        var con = confirm("정말 삭제하시겠습니까?");
        if(con) {
            var idx = $(this).parent().parent().attr('data-idx');
            var result = jsonreturn('/ad02/admin/productdel/'+idx);
