$(document).ready(function(){
  ...
  $('body').on('click', '#getcall', function() {
    var data = jsonreturn('/tmon/tmon/updateGet/'+$(this).attr('data-idx'));
  ...
  $('body').on('click', '.cate_list, .tmon_cate_list', function() {
    var cate_num = $(this).data('number');
    var content = $(this).attr( 'class' )

    var url = '';
    if (content.indexOf('tmon_cate_list') != -1) {
      url = '/tmon/tmon/getMainListCate/' + cate_num;
    } else {
