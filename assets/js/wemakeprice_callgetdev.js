$(document).ready(function(){
  ...
  $('body').on('click', '#getcall', function() {
    var data = jsonreturn('/wemakeprice/wemakeprice/updateGet/'+$(this).attr('data-idx'));
  ...
  $('body').on('click', '.cate_list, .wemakeprice_cate_list', function() {
    var cate_num = $(this).data('number');
    var content = $(this).attr( 'class' )

    var url = '';
    if (content.indexOf('wemakeprice_cate_list') != -1) {
      url = '/wemakeprice/wemakeprice/getMainListCate/' + cate_num;
    } else {
    ...
    $.ajax(
      ...
      $.each(data.list,function(index, item){
        ...
        html += '<div class="product_area" onclick="location.href=\'/wemakeprice/wemakeprice/get/' + item.product_idx + '\'">';
