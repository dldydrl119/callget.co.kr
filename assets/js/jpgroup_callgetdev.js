$(document).ready(function(){
  ...
  $('body').on('click', '#getcall', function() {
    var data = jsonreturn('/jpgroup/jpgroup/updateGet/'+$(this).attr('data-idx'));
  ...
  $('body').on('click', '.cate_list, .jpgroup_cate_list', function() {
    var cate_num = $(this).data('number');
    var content = $(this).attr( 'class' )

    var url = '';
    if (content.indexOf('jpgroup_cate_list') != -1) {
      url = '/jpgroup/jpgroup/getMainListCate/' + cate_num;
    } else {
    ...
    $.ajax(
      ...
      $.each(data.list,function(index, item){
        ...
        html += '<div class="product_area" onclick="location.href=\'/jpgroup/jpgroup/get/' + item.product_idx + '\'">';
